 ...
 * Se usa para evitar que cada parte del programa tenga que saber como se
 ...
 * <li><b>MONGO_DB</b>: nombre de la base de datos (por defecto:
 * {@code instituto}).</li>
 * <li><b>MONGO_COLLECTION</b>: nombre de la coleccion (por defecto:
 * {@code alumnos}).</li>
 * </ul>
 *
 * <h2>Nota:</h2>
 * <p>
 * MongoDB crea la base de datos y la coleccion automaticamente al insertar el
 * primer documento.
...
public final class MongoProvider implements AutoCloseable {
     ...
     * <p>
     * Lee la configuracion desde variables de entorno. Si alguna no esta definida,
    ...
    public MongoProvider() {
        ...
        String dbName = envOrDefault("MONGO_DB", "Empresa_Demo");
    ...
    /**
     * Permite crear una coleccion
     * 
     * @param nombreCollection
     */
    public void crearCollection(String nombreCollection) {
        try {
            if (getCollection(nombreCollection) == null) {
                envOrDefault("MONGO_COLLECTION", nombreCollection);
            }
        } catch (Exception e) {
            System.out.println("Erro al crear la coleccion: " + e.getMessage());
        }
    }

    /**
     * Permite seleccionar una coleccion.
     * 
     * @param nombreCollection Nombre de la coleccion
     * @return Devuelve la coleccion
     */
    public MongoCollection<Document> getCollection(String nombreCollection) {
        try {
            return database.getCollection(nombreCollection);
        } catch (Exception e) {
            System.out.println("Error al recuperar la coleccion: " + e.getMessage());
        }
        return null;
    }
    ...
    private static String envOrDefault(String key, String def) {
        ...
        if (variable == null || variable.isBlank()) {
            variable = def;// Devolvemos el valor por defecto pasado por parámetro si no recupera ningún
                           // valor
        }
        return variable;
    }
